...
export const getTasksLabel = (
	...
	const sceneCaptioningTasks = tasks.filter(
		(task) => task.type === 'scene-captioning',
	);
	...
	const failedSceneCaptioningTasks = sceneCaptioningTasks.filter(
		(task) => task.status.type === 'error',
	);
	...
	if (failedSceneCaptioningTasks.length > 0) {
		return {label: 'Scene captioning failed', progress: {type: 'error'}};
	}
	...
	const inProgressSceneCaptioningTasks = sceneCaptioningTasks.filter(
		(task) => task.status.type === 'processing',
	);
	...
	if (inProgressSceneCaptioningTasks.length > 0) {
		return {
			label: (
				<>
					Generating scenes
	...
	if (mostRecentTask.type === 'scene-captioning') {
		return {label: 'Scenes generated', progress: {type: 'success'}};
	}
