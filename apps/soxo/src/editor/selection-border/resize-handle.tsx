...
import {useDimensions, useItem} from '../utils/use-context';
...
const ResizeHandleUnmemoized: React.FC<{
	...
	const onPointerDown = useCallback(
		(e: React.MouseEvent) => {
			...
			const performResize = (pointerEvent: PointerEvent) => {
				const offsetX = (pointerEvent.clientX - initialX) / scale;
				const naturalOffsetY = (pointerEvent.clientY - initialY) / scale;
				ctrlPressed = pointerEvent.ctrlKey;
				...
				setState((state) => {
					changeItem(state, itemId, (i) => {
						...
						const lockAspectRatio =
							(FEATURE_SHIFT_KEY_TO_OVERRIDE_ASPECT_RATIO_LOCK &&
								pointerEvent.shiftKey) !== keepAspectRatio;
					...
					// Update snap points directly
					const shouldSnap =
						FEATURE_CANVAS_SNAPPING && snappingEnabled && !ctrlPressed;

					if (shouldSnap) {
						// Get the item from current timeline context (handles composite mode)
			...
			const onPointerMove = (pointerMoveEvent: PointerEvent) => {
				if (!isResizeActive) {
					...
					console.log('[RESIZE] START - first move');
					performResize(pointerMoveEvent);
					useEditorStore.temporal.getState().pause();
					isResizeActive = true;
					console.log('[RESIZE] Paused after first setState', {
			...
			const onPointerUp = () => {
				if (isResizeActive) {
					...
					console.log('[RESIZE] END - before resume', {
						pastCount: useEditorStore.temporal.getState().pastStates.length,
					});
					useEditorStore.temporal.getState().resume();
					isResizeActive = false;
					console.log('[RESIZE] Resumed', {
						pastCount: useEditorStore.temporal.getState().pastStates.length,
					});
				}

				// Clear snap points (paused so no undo point)
