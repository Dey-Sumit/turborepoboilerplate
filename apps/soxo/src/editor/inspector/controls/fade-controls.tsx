...
import {useItemFade} from '../../selectors/hooks';

const FadeControlsUnmemoized: React.FC<{
	itemId: string;
}> = ({itemId}) => {
	const {fadeInDurationInSeconds: fadeInDuration, fadeOutDurationInSeconds: fadeOutDuration, durationInFrames} = useItemFade(itemId);
	const {fps} = useFps();
