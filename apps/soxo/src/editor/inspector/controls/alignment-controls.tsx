		...
		setState((state) => {
			changeItem(state, itemId, (i) => {
				...
				const targetLeft = Math.round(
					(state.compositionState.compositionWidth - rectAfterCrop.width) / 2,
				);
	...
	const onRight = React.useCallback(() => {
		setState((state) => {
			changeItem(state, itemId, (i) => {
				const rectAfterCrop = getRectAfterCrop(i);
				const targetLeft =
					state.compositionState.compositionWidth - rectAfterCrop.width;
				const currentLeft = rectAfterCrop.left;
				const offset = targetLeft - currentLeft;
	...
	const onBottom = React.useCallback(() => {
		setState((state) => {
			changeItem(state, itemId, (i) => {
				const rectAfterCrop = getRectAfterCrop(i);
				const targetTop =
					state.compositionState.compositionHeight - rectAfterCrop.height;
		...
		setState((state) => {
			changeItem(state, itemId, (i) => {
				...
				const targetTop = Math.round(
					(state.compositionState.compositionHeight - rectAfterCrop.height) / 2,
				);
				const currentTop = rectAfterCrop.top;
				const offset = targetTop - currentTop;
