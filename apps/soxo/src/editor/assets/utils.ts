...
export const getAssetDurationInSeconds = (asset: EditorStarterAsset) => {
	...
	if (asset.type === 'scene-caption') {
		// Scene caption duration is based on the last segment's end time
		const lastSegment = asset.segments[asset.segments.length - 1];
		return lastSegment ? lastSegment.adjustedEndMs / 1000 : 0;
	}
