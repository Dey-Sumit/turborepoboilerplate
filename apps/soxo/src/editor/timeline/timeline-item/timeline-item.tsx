...
import {TimelineItemData} from '../../selectors/types';
...
import {useSelectedItemsSet} from '../../../zustand/ui-store';
...
import {TimelineItemTransitionIndicator} from './timeline-item-transition-indicator';
import {getWaveformHeight} from './timeline-item-waveform';

type TimelineItemProps = {
	item: EditorStarterItem | TimelineItemData; // Accept both full item and timeline slice
			...
			<ItemContextMenuTrigger item={item}>
				<div ref={hoverRef} style={style}>
					...
					<TimelineItemExtendHandles
						...
						height={height}
					/>
					{/* Render transition indicator if this item has a transition to next */}
					{item.transition?.toNext !== undefined && (
						<TimelineItemTransitionIndicator />
					)}
					{shouldShowAudioFadeControl(item.type) ? (
						<ItemFadeHandles
...
export const TimelineItem = memo(
	({
		...
		if (item.isDraggingInTimeline) {
			console.log('[TIMELINE ITEM] Hidden because isDraggingInTimeline = true', {
				itemId: item.id,
				from: item.from,
