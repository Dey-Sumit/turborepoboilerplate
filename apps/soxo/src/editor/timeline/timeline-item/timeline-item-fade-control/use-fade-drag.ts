...
export const useFadeDrag = ({
	...
	const handleMouseDown = useCallback(
		(e: React.MouseEvent) => {
			...
			const initialItem = currentItems[item.id] as EditorStarterItem;
			...
			const handlePointerMove = (evt: MouseEvent) => {
				...
				// Update fade during drag - should NOT be added to undo history
			...
			const handlePointerUp = (evt: PointerEvent) => {
				...
				// Final fade value SHOULD be added to undo history
				setState((state) => {
					changeItem(state, item.id, (prevItem) => {
						if (
							prevItem.type !== 'audio' &&
						...
						if (getFadeValue({item: prevItem, fadeProperty}) === newFadeValue) {
							return prevItem;
						}
