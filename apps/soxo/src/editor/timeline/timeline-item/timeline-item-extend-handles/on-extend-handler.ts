...
export const onExtendHandler = ({
	...
	setState: (update: (draft: EditorState) => void) => void;
	...
	const {compositionState} = currentState;
	// Get items and tracks from current timeline context (could be inside a composite)
	const timeline = getCurrentTimeline(compositionState);
	const {tracks, items} = timeline;
	const {fps, assets} = compositionState;
	...
	// Mark items as being trimmed - should NOT be added to undo history
	...
	const compositionDurationInFrames = getCompositionDuration(items, tracks);
	...
	const fromPointerEvent = (
		...
		if (isSnappingEnabled && newSelectedItems.length > 0) {
		...
		// Apply trim during drag - commitToUndoStack determines if added to undo history
		if (!commitToUndoStack) {
			const temporalState = useEditorStore.temporal.getState();
			temporalState.pause();
		}
		...
		setState((state) => {
			for (const itemId of newSelectedItems) {
				...
				const trackItemsSorted = tracks[trackIndex].items
					...
					.sort((a, b) => items[a].from - items[b].from);
				if (extend.type === 'left') {
					changeItem(state, item.id, (prevItem) => {
						return extendLeft({
							prevItem,
							trackItemsSorted,
	...
	const onPointerUp = (pointerUpEvent: PointerEvent) => {
		...
		// Unmark items as being trimmed - should NOT be added to undo history
		const temporalState = useEditorStore.temporal.getState();
		temporalState.pause();
